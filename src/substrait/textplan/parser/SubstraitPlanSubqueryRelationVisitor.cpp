  ...
  if (parentRelationType == RelationType::kRead) {
    ...
    if (symbol != nullptr) {
      ...
      for (const auto& sym : *symbolTable_) {
        ...
        if (sym.sourceLocation != symbol->sourceLocation) {
          continue;
        }
...
std::any SubstraitPlanSubqueryRelationVisitor::visitRelationEmit(
  ...
  if (stepsOut > 0) {
    errorListener_->addError(
        ctx->getStart(), "Emitting outer references is not supported.");
    return defaultResult();
  }
  common->mutable_emit()->add_output_mapping(fieldReference);
  return defaultResult();
}
...
std::any SubstraitPlanSubqueryRelationVisitor::visitRelationSourceReference(
  ...
  switch (ANY_CAST(SourceType, symbol->subtype)) {
    ...
    case SourceType::kNamedTable: {
      ...
      for (const auto& sym :
           symbolTable_->lookupSymbolsByLocation(symbol->sourceLocation)) {
        if (sym->type != SymbolType::kSourceDetail) {
          continue;
        }
...
std::any SubstraitPlanSubqueryRelationVisitor::visitExpressionColumn(
  ...
  if (fieldReference != -1) {
    ...
    // TODO -- Update this with other kinds of references.
    if (stepsOut > 0) {
      expr.mutable_selection()->mutable_outer_reference()->set_steps_out(
          stepsOut);
    } else {
...
std::any SubstraitPlanSubqueryRelationVisitor::visitExpression_list(
  ...
  for (auto exprCtx : ctx->expression()) {
    auto expr =
        ANY_CAST(::substrait::proto::Expression, visitExpression(exprCtx));
...
std::any SubstraitPlanSubqueryRelationVisitor::visitColumn_name(
  ...
  return findFieldReferenceByName(
      ctx->getStart(), currentRelationScope_, relationData, ctx->getText());
}
...
std::string SubstraitPlanSubqueryRelationVisitor::fullyQualifiedReference(
  ...
  for (const auto& symbol : symbolTable_->getSymbols()) {
    if (symbol->type == SymbolType::kSchema &&
        symbol->sourceLocation == fieldReference->sourceLocation) {
...
SubstraitPlanSubqueryRelationVisitor::findFieldReferenceByName(
  ...
  if (generatedField != relationData->generatedFieldReferences.rend()) {
    ...
    return {
        ...
        static_cast<int32_t>(
            (fieldReferencesSize +
             relationData->generatedFieldReferences.size() - fieldPlacement -
             1) &
...
bool SubstraitPlanSubqueryRelationVisitor::isWithinSubquery(
  ...
  if (symbol->parentQueryLocation != Location::kUnknownLocation) {
    return true;
  }
