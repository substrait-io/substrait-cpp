...
std::string RelTypeCaseName(::substrait::proto::Rel::RelTypeCase num) {
  ...
  if (num >= case_names.size()) {
    // The base visitor will raise an error for us so return something simple.
    return "error";
  }
...
std::any PlanPrinterVisitor::visitReadRelation(
  ...
  if (relation.has_best_effort_filter()) {
    text << "  filter " <<
        ANY_CAST(std::string, visitExpression(relation.best_effort_filter())) <<
        ";\n";
  }
...
std::any PlanPrinterVisitor::visitAggregateRelation(
  ...
  for (const auto& group : relation.groupings()) {
    for (const auto& expr : group.grouping_expressions()) {
      text <<
          "  grouping " << ANY_CAST(std::string, visitExpression(expr)) << ";\n";
    }
  }
