...
SubstraitTypePtr SubstraitType::decode(const std::string& rawType) {
  ...
  if (SubstraitTypeTraits<SubstraitTypeKind::kList>::typeString == baseType) {

    return std::make_shared<SubstraitListType>(nestedTypes[0]);
  } else if (
      SubstraitTypeTraits<SubstraitTypeKind::kMap>::typeString == baseType) {

    return std::make_shared<SubstraitMapType>(nestedTypes[0], nestedTypes[1]);
  } else if (
      SubstraitTypeTraits<SubstraitTypeKind::kDecimal>::typeString ==
      baseType) {
    ...
    return std::make_shared<SubstraitFixedCharType>(length);
      ...
      SubstraitTypeTraits<SubstraitTypeKind::kFixedBinary>::typeString ==
      baseType) {
    ...
    return std::make_shared<SubstraitFixedBinaryType>(length);
      ...
      SubstraitTypeTraits<SubstraitTypeKind::kStruct>::typeString == baseType) {
