...
protected:
  ...
  template <class T>
  void testDecode(const std::string &rawType,
...
TEST_F(TypeTest, decodeTest) {
  ...
  testDecode<FixedCharType>(
      "fixedchar<L1>", [](const std::shared_ptr<const FixedCharType> &typePtr) {
  ...
  testDecode<VarcharType>(
      "varchar<L1>", [](const std::shared_ptr<const VarcharType> &typePtr) {
        ASSERT_EQ(typePtr->signature(), "vchar<L1>");
        ASSERT_EQ(typePtr->length()->value(), "L1");
      });

  testDecode<DecimalType>(
      "decimal<P,S>", [](const std::shared_ptr<const DecimalType> &typePtr) {
        ASSERT_EQ(typePtr->signature(), "dec<P,S>");
        ASSERT_EQ(typePtr->precision(), "P");
        ASSERT_EQ(typePtr->scale(), "S");
      });

  testDecode<StructType>("struct<i32,i64,string>",
                         [](const std::shared_ptr<const StructType> &typePtr) {
                           ASSERT_EQ(typePtr->signature(),
                                     "struct<i32,i64,str>");
                         });

  testDecode<StructType>("struct<i32,i64,struct<string,string>>",
                         [](const std::shared_ptr<const StructType> &typePtr) {
                           ASSERT_EQ(typePtr->signature(),
                                     "struct<i32,i64,struct<str,str>>");
                         });

  testDecode<ListType>("list<string>",
                       [](const std::shared_ptr<const ListType> &typePtr) {
                         ASSERT_EQ(typePtr->signature(), "list<str>");
                       });

  testDecode<MapType>("map<string,i32>",
                      [](const std::shared_ptr<const MapType> &typePtr) {
                        ASSERT_EQ(typePtr->signature(), "map<str,i32>");
                      });

  testDecode<StringLiteralType>(
      "any1", [](const std::shared_ptr<const StringLiteralType> &typePtr) {
        ASSERT_EQ(typePtr->signature(), "any1");
        ASSERT_TRUE(typePtr->isWildcard());
      });

  testDecode<StringLiteralType>(
      "any", [](const std::shared_ptr<const StringLiteralType> &typePtr) {
        ASSERT_EQ(typePtr->signature(), "any");
        ASSERT_TRUE(typePtr->isWildcard());
      });

  testDecode<StringLiteralType>(
      "T", [](const std::shared_ptr<const StringLiteralType> &typePtr) {
